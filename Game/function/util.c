...
int getObjStateInRoom(char *name) {
	...
	if (fd == -1) {
		write(2, "Could not obtain the get of the object. Error opening the file.\n", strlen("Coud not get the state of the object. Error opening the file.\n"));
		exit(1);
	}
...
void setObjStateInRoom(char *name, int newState) {
	...
	fd = open(object, O_WRONLY);
        if (fd == -1) {
                write(2, "Could not set the state of the object. Error opening the file.\n", strlen("Coud not set the state of the object. Error opening the file.\n"));
                exit(1);
        }
